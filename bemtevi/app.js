...
// Novas variáveis para recursos expandidos
...
async function isPostSaved(postId) {
    ...
    try {
        const doc = await db.collection('usuarios').doc(currentUser.uid)
...
async function createCommunity(name, description, category) {
    ...
    if (!name || name.length < 3) {
        alert('O nome da comunidade deve ter pelo menos 3 caracteres.');
        return null;
    }
...
async function getTrendingTopics(limit = 10) {
    try {
        // Buscar posts das últimas 24h e agrupar por palavras-chave
        ...
        snapshot.forEach(doc => {
            ...
            // Extrair palavras-chave (2-3 palavras)
...
async function sendMessage(receiverId, message) {
    ...
    try {
        ...
        await db.collection('usuarios').doc(currentUser.uid)
            .collection('chats').doc(receiverId).set({
                chatId: chatId,
                lastMessage: message.trim(),
                lastMessageTime: firebase.firestore.FieldValue.serverTimestamp()
            }, { merge: true });
            
        await db.collection('usuarios').doc(receiverId)
            .collection('chats').doc(currentUser.uid).set({
                chatId: chatId,
                lastMessage: message.trim(),
                lastMessageTime: firebase.firestore.FieldValue.serverTimestamp()
            }, { merge: true });
...
async function getMessages(userId, limit = 50) {
    ...
    try {
        ...
        for (const msg of unread) {
            await db.collection('chats').doc(chatId)
                .collection('messages').doc(msg.id).update({ read: true });
        }
...
async function createPost(conteudo, link = null, categoria = 'Geral', communityId = null) {
    ...
    try {
        ...
        const docRef = await db.collection('Bemtevi').add(postData);
        
        // Atualizar contagem de posts da comunidade
        if (communityId) {
...
async function loadPosts(reset = false) {
    ...
    try {
        ...
        } else if (currentFeed === 'community' && currentCommunity) {
            query = db.collection('Bemtevi').where('communityId', '==', currentCommunity).orderBy('createdAt', 'desc');
                    ...
                    <div class="post-header">
                        ...
                        <div class="post-options">
                            ...
                            <div class="post-options-dropdown" id="options-${post.id}">
                                ...
                                `}
                                <button onclick="savePost('${post.id}')">${isSaved ? '📁 Remover dos salvos' : '📁 Salvar'}</button>
                            </div>
                        </div>
                    </div>
            ...
            <div class="sidebar-left">
                <div class="card">
                    ...
                    <ul class="nav-menu">
                        ...
                        <li class="nav-item" onclick="openProfile('${currentUser.uid}', '${currentUser.displayName || currentUser.email.split('@')[0]}')">
...
window.submitPost = async function() {
    ...
    if (await createPost(text, link, category, currentCommunity)) {
        const postText = document.getElementById('postText');
