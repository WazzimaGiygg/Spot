...
function getUrlParams() {
    ...
    return { 
        uid: params.get('uid'), 
        info: params.has('info'),
        page: parseInt(params.get('page')) || 1
    };
}

function getBlogUrl(uid, page = 1) {
    const baseUrl = window.location.origin + window.location.pathname;
    return `${baseUrl}?uid=${uid}&page=${page}`;
}
...
// ============================================
function navigateToPage(uid, page) {
    const baseUrl = window.location.pathname;
    const url = `${baseUrl}?uid=${uid}&page=${page}`;
    window.history.pushState({ uid, page }, '', url);
    renderUserBlog(uid, page);
}

// ============================================
// COMPARTILHAMENTO
// ============================================
function shareBlog(uid, platform, page = 1) {
    const url = getBlogUrl(uid, page);
...
function renderBlog() {
    ...
    if (urlUid) {
        renderUserBlog(urlUid, page);
        return;
    }

    if (currentUser) {
        renderUserBlog(currentUser.uid, page);
        return;
    }
...
function setupEditor(uid) {
    ...
    postContentInput.addEventListener('input', window.updatePreview);
...
function navigateToHome() {
    const params = getUrlParams();
    if (params.uid) {
        window.location.href = window.location.pathname + '?uid=' + params.uid + '&page=1';
    } else {
...
console.log('📄 Paginação: 1 post por página');
